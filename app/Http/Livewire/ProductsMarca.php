...
class ProductsMarca extends Component
{

    public $marca, $products, $productsConst, $search;

    public function mount()
    {
        $this->products = $this->marca->products;
    ...
    public function render()
    {
        if ($this->search != "") {
            $this->products = $this->productsConst->filter(function ($value, $key) {
                ...
                return strpos(strtoupper($value->nombre), strtoupper($this->search)) !== false || strpos(strtoupper($value->sku), strtoupper($this->search)) !== false;
            });
        } else {
            $this->products = $this->productsConst;
        }
        return view('livewire.products-marca');
    }
}
